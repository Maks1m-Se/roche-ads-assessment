...
| 4 | Q3 | Denominator for percentages | **Subjects per arm from `adsl`, not from `adae`.** Subjects per treatment arm from `pharmaverseadam::adsl`, not subjects with AEs from `adae`. This is why the spec lists `adsl` as a Q3 input. |
